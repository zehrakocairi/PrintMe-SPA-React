...
const useFetch = (route, onReceived) => {
  ...
  if (route.includes("api/")) {
     ...
     */
    throw Error("when using the useFetch hook, the route should not include the /api/ part");
  }
  ...
  const performFetch = (options, newUrl) => {
    ...
    const fetchData = async () => {
      ...
      if (!res.ok) {
        setError(`Fetch for ${url} returned an invalid status (${res.status}). Received: ${JSON.stringify(res)}`);
      }
      ...
      if (jsonResult.success === true) {
        ...
        setError(jsonResult.msg || `The result from our API did not have an error message. Received: ${JSON.stringify(jsonResult)}`);
      }
