...
const ProductQuickView: FC<ProductQuickViewProps> = ({ item, className = "" }) => {
  ...
  }, [selectedSizeIndex, selectedFrameIndex]);

  const notifyAddTocart = () => {
    var variantIndex = 0; // TODO : Implement here
    ...
    cartItem.quantity = qualitySelected;
    cartItem.pictureUrl = item.imageThumbnail;

    addItemToCart(cartItem);
    toast.custom((t) => <NotifyAddTocart product={item} qualitySelected={qualitySelected} show={t.visible} sizeSelected={sizes[selectedSizeIndex]} calculatedPrice={calculatedPrice} />, { position: "top-right", id: "nc-product-notify", duration: 3000 });
  };
        ...
        <div className="flex justify-between font-medium text-sm">
          <label className="rtl:text-right block" htmlFor="">
            <span className="text-sm font-medium">
              Color:
              <span className="ms-1 font-semibold">{frames[selectedFrameIndex].name}</span>
            </span>
          </label>
          <a target="_blank" rel="noopener noreferrer" className="text-primary-6000 hover:text-primary-500" href="/our-services">
            {t("See frame details")}
          </a>
        </div>
        <div className="grid grid-cols-6 gap-2 mt-3">
          {frames.map((frame, index) => (
            <div title={frame.name} key={index} onClick={() => setSelectedFrameIndex(index)} className={`relative flex max-w-[75px] h-16 rounded-sm md:rounded-lg border-2 cursor-pointer ${selectedFrameIndex === index ? "border-primary-6000 dark:border-primary-500" : "border-transparent"}`}>
              <div
    ...
    return (
      <div>
        <div className="flex justify-between font-medium text-sm">
          ...
          <a target="_blank" rel="noopener noreferrer" className="text-primary-6000 hover:text-primary-500">
            {t("See sizing chart")}
          </a>
        </div>
          ...
          {sizes.map((size, index) => {
            ...
            return (
              <div
                ...
                text-sm  font-semibold select-none overflow-hidden z-0 cursor-pointer ${isActive ? "bg-primary-6000 border-primary-6000 text-white hover:bg-primary-6000" : "border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-200 hover:bg-neutral-50 dark:hover:bg-neutral-700"}`}
                onClick={() => {
  ...
  const renderStatus = () => {
    const status = "New in"; // TODO : Fix here
    if (!status) {
      return null;
    }
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <div className="flex justify-start rtl:justify-end items-center mt-5 space-x-4 sm:space-x-5 rtl:space-x-reverse">
            ...
            <Prices contentClass="py-1 px-2 md:py-1.5 md:px-3 text-lg font-semibold" price={calculatedPrice} />
            <div className="h-6 border-s border-slate-300 dark:border-slate-700"></div>

            <div className="flex items-center">
              <Link href={`/product-detail/${id}`} className="flex items-center text-sm font-medium">
                <StarIcon className="w-5 h-5 pb-[1px] text-yellow-400" />
                <div className="ms-1.5 flex">
                  {rating || ""}
                  <span className="block mx-2">·</span>
                  <span className="text-slate-600 dark:text-slate-400 underline">{numberOfReviews} reviews</span>
                </div>
              </Link>
        ...
        <div className="w-full lg:w-[50%] ">
          ...
          <div className="relative">
            <div className="aspect-w-16 aspect-h-16">
              <Image src={item.image} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="w-full rounded-xl object-cover" alt="product detail 1" />
            </div>
            ...
            {[item.image2, item.image3].map((item, index) => {
              return (
                <div key={index} className="aspect-w-3 aspect-h-4">
                  <Image fill src={item} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="w-full rounded-xl object-cover" alt="product detail 1" />
                </div>
              );
            })}
          </div>
