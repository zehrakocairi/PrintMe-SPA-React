...
const TabFilters = () => {
  ...
  const handleChangeCategories = (checked: boolean, cat: Category) => {
    let newState = checked ? (filter.categoryState ?? cat) | cat : (filter.categoryState ?? cat) & ~cat;
            ...
            <Popover.Button
              className={`flex items-center justify-center px-4 py-2 text-sm rounded-full border focus:outline-none select-none
               ${open ? "!border-primary-500 " : "border-neutral-300 dark:border-neutral-700"}
                ${!!filter.categoryState !== undefined ? "!border-primary-500 bg-primary-50 text-primary-900" : "border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 hover:border-neutral-400 dark:hover:border-neutral-500"}
                `}
            >
                  ...
                  <div className="relative flex flex-col px-5 py-6 space-y-3">
                    {DATA_categories.map((item) => (
                      <div key={item.name} className="">
                        <Checkbox name={item.name} label={item.name} defaultChecked={((filter.categoryState ?? Category.None) & item.value) === item.value} onChange={(checked) => handleChangeCategories(checked, item.value)} />
                      </div>
                    ))}
                  </div>
          ...
          <>
            <Popover.Button
              ...
              ${open ? "!border-primary-500 " : ""}
                ${!!filter.sortOrderStates.length ? "!border-primary-500 bg-primary-50 text-primary-900" : "border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 hover:border-neutral-400 dark:hover:border-neutral-500"}
                `}
            >
              ...
              <span className="ml-2">{filter.sortOrderStates ? DATA_sortOrderRadios.filter((i) => i.id === filter.sortOrderStates)[0].name : t("Sort order")}</span>
              {!filter.sortOrderStates.length ? <ChevronDownIcon className="w-4 h-4 ml-3" /> : <span onClick={() => updateSortOrderStates("")}>{renderXClear()}</span>}
            </Popover.Button>
            <Transition as={Fragment} enter="transition ease-out duration-200" enterFrom="opacity-0 translate-y-1" enterTo="opacity-100 translate-y-0" leave="transition ease-in duration-150" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 translate-y-1">
                ...
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                  <div className="relative flex flex-col px-5 py-6 space-y-5">
                    {DATA_sortOrderRadios.map((item) => (
                      <Radio id={item.id} key={item.id} name="radioNameSort" label={item.name} defaultChecked={filter.sortOrderStates === item.id} onChange={updateSortOrderStates} />
                    ))}
                  </div>
                  <div className="p-5 bg-neutral-50 dark:bg-neutral-900 dark:border-t dark:border-neutral-800 flex items-center justify-between">
                    <ButtonThird
  ...
  //                         name={item.name}
  //                         label={item.name}
  //                         defaultChecked={(filter.sizeState ?? PrintSize.None & item.value) == item.value }
  //                         onChange={(checked) =>
  //                           handleChangeSizes(checked, item.value)
  //                         }
  //                       />
  //                     </div>
  //                   ))}
  ...
  const renderTabsPriceRage = () => {
    return (
      <Popover className="relative">
        {({ open, close }) => (
          <>
            <Popover.Button className={`flex items-center justify-center px-4 py-2 text-sm rounded-full border border-primary-500 bg-primary-50 text-primary-900 focus:outline-none `}>
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
              ...
              {filter.rangePrices[0] === PRICE_RANGE[0] && filter.rangePrices[1] === PRICE_RANGE[1] ? null : <span onClick={() => updateRangePrices(PRICE_RANGE)}>{renderXClear()}</span>}
            </Popover.Button>
            <Transition as={Fragment} enter="transition ease-out duration-200" enterFrom="opacity-0 translate-y-1" enterTo="opacity-100 translate-y-0" leave="transition ease-in duration-150" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 translate-y-1">
                ...
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                  <div className="relative flex flex-col px-5 py-6 space-y-8">
                    <div className="space-y-5">
                      ...
                      <Slider range min={PRICE_RANGE[0]} max={PRICE_RANGE[1]} step={1} defaultValue={[filter.rangePrices[0], filter.rangePrices[1]]} allowCross={false} onChange={(_input: number | number[]) => updateRangePrices(_input as number[])} />
                    </div>
                    ...
                    <div className="flex justify-between space-x-5">
                      <div>
                        ...
                        <div className="mt-1 relative rounded-md">
                          ...
                          <input type="text" name="minPrice" disabled id="minPrice" className="block w-32 pr-10 pl-4 sm:text-sm border-neutral-200 dark:border-neutral-700 rounded-full bg-transparent" value={filter.rangePrices[0]} />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="mt-1 relative rounded-md">
                          <span className="absolute inset-y-0 right-4 flex items-center pointer-events-none text-neutral-500 sm:text-sm">€</span>
                          <input type="text" disabled name="maxPrice" id="maxPrice" className="block w-32 pr-10 pl-4 sm:text-sm border-neutral-200 dark:border-neutral-700 rounded-full bg-transparent" value={filter.rangePrices[1]} />
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  const renderTabIsOnsale = () => {
    return (
      <div
        className={`flex items-center justify-center px-4 py-2 text-sm rounded-full border focus:outline-none cursor-pointer select-none ${filter.isOnSale ? "border-primary-500 bg-primary-50 text-primary-900" : "border-neutral-300 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 hover:border-neutral-400 dark:hover:border-neutral-500"}`}
        onClick={() => updateIsOnSale(!filter.isOnSale)}
      >
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
  ...
  const renderTabMobileFilter = () => {
    return (
      <div className="flex-shrink-0">
        <div className={`flex flex-shrink-0 items-center justify-center px-4 py-2 text-sm rounded-full border border-primary-500 bg-primary-50 text-primary-900 focus:outline-none cursor-pointer select-none`} onClick={openModalMoreFilter}>
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div className="flex-grow overflow-y-auto">
                    <div className="px-6 sm:px-8 md:px-10 divide-y divide-neutral-200 dark:divide-neutral-800">
                      ...
                      <div className="py-7">
                        ...
                        <div className="mt-6 relative ">
                          <div className="relative flex flex-col space-y-8">
                            <div className="space-y-5">
                              <Slider range className="text-red-400" min={PRICE_RANGE[0]} max={PRICE_RANGE[1]} defaultValue={filter.rangePrices} allowCross={false} onChange={(_input: number | number[]) => updateRangePrices(_input as number[])} />
                            </div>
                            ...
                            <div className="flex justify-between space-x-5">
                              <div>
                                ...
                                <div className="mt-1 relative rounded-md">
                                  ...
                                  <input type="text" name="minPrice" disabled id="minPrice" className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-3 sm:text-sm border-neutral-200 rounded-full text-neutral-900" value={filter.rangePrices[0]} />
                                </div>
                              </div>
                              <div>
                                <label htmlFor="maxPrice" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                ...
                                <div className="mt-1 relative rounded-md">
                                  ...
                                  <input type="text" disabled name="maxPrice" id="maxPrice" className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-3 sm:text-sm border-neutral-200 rounded-full text-neutral-900" value={filter.rangePrices[1]} />
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="py-7">
                        ...
                        <div className="mt-6 relative ">
                          <MySwitch label={t("On sale") + "!"} desc={t("Products currently on sale")} enabled={filter.isOnSale} onChange={updateIsOnSale} />
                        </div>
                      </div>
                    </div>
                  </div>
