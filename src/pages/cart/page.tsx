...
const CartPage = () => {
  ...
  const renderProduct = (item: CartItem, index: number) => {
    ...
    return (
      <div key={index} className="relative flex py-8 sm:py-10 xl:py-12 first:pt-0 last:pb-0">
        <div className="relative h-36 w-24 sm:w-32 flex-shrink-0 overflow-hidden">
          <Image fill src={pictureUrl} alt={productName} sizes="300px" className="h-full w-full object-contain object-center" />
          <Link href={`/product-detail/${productId}`} className={`absolute inset-0 ${productName == "Custom Product" ? "hidden" : ""}`}></Link>
        </div>
        ...
        <div className="ml-3 sm:ml-6 flex flex-1 flex-col">
            ...
            <div className="flex justify-between ">
              <div className="flex-[1.5] ">
                <h3 className="text-base font-semibold">
                  <Link href={`${productName == "Custom Product" ? "" : `/product-detail/${productId}`}`}>{productName}</Link>
                </h3>
                ...
                <div className="mt-3 flex justify-between w-full sm:hidden relative">
                  <Prices hideFromLabel={true} contentClass="py-1 px-2 md:py-1.5 md:px-2.5 text-sm font-medium h-full" price={unitPrice * quantity} />
                </div>
              </div>

              <div className="hidden sm:block text-center relative">
                <NcInputNumber defaultValue={quantity} onChange={setQuantity} className="relative z-10" />
              </div>
          ...
          <div className="flex mt-auto pt-4 items-end justify-between text-sm">
            ...
            <a href="##" className="relative z-10 flex items-center mt-3 font-medium text-primary-6000 hover:text-primary-500 text-sm ">
              <span
                onClick={() => {
                  removeItemFromCart(productId, frameId, size, pictureUrl);
                }}
              >
                Remove
              </span>
  ...
  return (
    <div className="nc-CartPage">
      <Helmet>
        <title>PrintMeArt - {t("Cart")}</title>
        <link rel="canonical" href={`${window.location.origin}/cart?lang=${i18n.language}`} />
      </Helmet>
      <main className="container py-16 lg:pb-28 lg:pt-20 ">
        <div className="mb-12 sm:mb-16">
          <h2 className="block text-2xl sm:text-3xl lg:text-4xl font-semibold ">{t("Shopping Cart")}</h2>
        ...
        <div className="flex flex-col lg:flex-row">
          <div className="w-full lg:w-[60%] xl:w-[55%] divide-y divide-slate-200 dark:divide-slate-700 ">{cart?.map((item, index) => renderProduct(item, index))}</div>
            ...
            <div className="sticky top-28">
              ...
              <div className="mt-7 text-sm text-slate-500 dark:text-slate-400 divide-y divide-slate-200/70 dark:divide-slate-700/80">
                <div className="flex justify-between pb-4">
                  ...
                  <span className="font-semibold text-slate-900 dark:text-slate-200">€{cartTotal}</span>
                </div>
                <div className="flex justify-between py-4">
                  <span>{t("Shipping estimate")}</span>
                  <span className="font-semibold text-slate-900 dark:text-slate-200">€{shippingPrice}</span>
                </div>
                <div className="flex justify-between py-4">
                  <span>{t("Tax estimate")}</span>
                  <span className="font-semibold text-slate-900 dark:text-slate-200">€{taxTotal.toFixed(2)}</span>
                </div>
              ...
              <div className="mt-5 text-sm text-slate-500 dark:text-slate-400 flex items-center justify-center">
                <p className="block relative pl-5">
                  ...
                  {t("Learn more ")}
                  <a aria-label="Learn More about PrintMe" target="_blank" rel="noopener noreferrer" className="text-slate-900 dark:text-slate-200 underline font-medium">
                    {t("Taxes")}
                  </a>
                  <span>{t(" and ")}</span>
                  <a target="_blank" rel="noopener noreferrer" className="text-slate-900 dark:text-slate-200 underline font-medium">
                    {t("Shipping ")}
                  </a>
                  {t("infomation")}
                </p>
