...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const addItemToCart = async (item: CartItem) => {
    ...
    trackEvent("Add to Cart", `Product: ${item.productName}`);
    let updatedCart = [...cart];

    const existingItem = updatedCart.find((cartItem) => cartItem.productId === item.productId && item.pictureUrl === cartItem.pictureUrl);
  ...
  const removeItemFromCart = async (productId: number, frameId: number, size: PrintSize, pictureUrl: string) => {
    const updatedCart = cart.filter((item) => item.productId !== productId || item.pictureUrl !== pictureUrl);
    setCart(updatedCart ?? []);
    localStorage.setItem("cart", JSON.stringify(updatedCart));
  ...
  return <CartContext.Provider value={{ cart, addItemToCart, removeItemFromCart, cartTotal, taxTotal }}>{children}</CartContext.Provider>;
};
